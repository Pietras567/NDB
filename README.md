...
This project is a **Car Rental Application** that uses **MongoDB** as the primary database. The application is built using **Java** and managed with **Gradle**.
...
3. **MongoDB** servers running locally or use docker-compose file.

---

## Cloning the Repository

To clone the project repository `MongoDB` branch:

```bash
git clone https://github.com/Pietras567/NDB.git --branch MongoDB
cd NDB
```
...
2. Configure the database and cache manager (if needed):
   - Update the **MongoDB** configuration details in `mongod.conf`.
